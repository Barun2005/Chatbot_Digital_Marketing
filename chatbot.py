...
if not api_key:
    ...
    api_key = "AIzaSyCbIedyqraZAZV_Dm8iN0H6wp2Ek1yiTcU"
...
def chat():
    ...
    try:
        ...
        # Define the system prompt for digital marketing focus
        system_prompt = """You are a Digital Marketing Expert Bot designed to educate users on all aspects of digital marketing. You have two main modes of interaction:

1. INFORMATION MODE: When users ask general questions about digital marketing topics, provide clear, concise, and educational responses. Keep explanations brief (maximum 3-4 short paragraphs) and focus on the most important points. Topics include SEO, social media marketing, content marketing, email marketing, PPC advertising, influencer marketing, analytics, conversion optimization, marketing automation, branding, e-commerce marketing, mobile marketing, video marketing, and digital marketing strategies.
...
COMPARISON FORMAT: When users ask about differences or comparisons between two or more concepts (e.g., "What's the difference between SEO and PPC?"), format your response as an HTML table with columns. Each column should have a header with the concept name, and the content should be in bullet points. For example:
  ...
  <tr>
    <td>
      <ul>
        ...
        <li>No direct cost per click</li>
      </ul>
    </td>
    <td>
      <ul>
...
Analyze each user message carefully to determine which mode is appropriate. If the message contains a direct question about digital marketing, use INFORMATION MODE. If the message explicitly requests a quiz or question, use QUIZ MODE.
